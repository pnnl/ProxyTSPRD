...
class PyTorchInterfaceGPU(PyTorchInterface):
    ...
    def infer_on_data(
        ...
        for inputs, target in test_data:
            ...
            if inputs.shape[0] == batch_size:
                ...
                if infer_through=="ait":
                ...
                elif re.match("onnx.*", infer_through):
                    if (input_nodes is not None) or (output_nodes is not None) or (mfgs is not None):
                    ...
                    # print("[INFO (ONNX)] Inferred through %s" %(infer_through))
                else:
                    # move data to GPU
            ...
            # free cuda memory
            memory_free_time = time.perf_counter() 
            del inputs
            del target
            extra_time += time.perf_counter() - memory_free_time
    ...
    def infer(
        ...
        # go through all the batches generated by dataloader
        if graphloader is None:
            test_loss, num_samples, surplus_time = self.infer_on_data(
                data, 
            ...
            surplus_time = 0 
            for input_nodes, output_nodes, mfgs in graphloader:
                graph_loss, graph_samples, extra_time = self.infer_on_data(
                    data, 
                    infer_through=infer_through,
                    batch_size=batch_size, 
        ...
        if self._GLOBAL_RANK == 0:
            ...
            print("============> (Before Sync) Inference Time:", inf_time-surplus_time)
            print("============> (Before Sync) Inference Loss:", test_loss / num_samples)

        # sum of all the times
        if self._MGPU_STRATEGY == "HVD":
            inf_time = self.hvd_torch.allreduce(inf_time)
            surplus_time = self.hvd_torch.allreduce(surplus_time)
            ...
            surplus_time = surplus_time / self._MGPU_SIZE
        
        if self._GLOBAL_RANK == 0:
            if self._DEVICE_STR == "cuda":
                torch.cuda.synchronize()
                cuda_time = start.elapsed_time(end) / 1000
                print("============> (After Sync) Inference CUDA Time (Total): %f" %(cuda_time))
                print("============> (After Sync) Inference CUDA Time: %f" %(cuda_time-surplus_time))
            print("============> (After Sync) Inference Time (Total): ", inf_time)
            print("============> (After Sync) Inference Time: ", inf_time-surplus_time)
