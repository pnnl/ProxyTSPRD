...
for s_dir in dir_list:
    ...
    original_shape = np.concatenate([dataset.F for i in range(repeat)] +  
                                    [dataset.Vm for i in range(repeat)], axis=1).shape
    scenario_data.append(np.concatenate([dataset.F[:1400,:] for i in range(repeat)] + 
                                        [dataset.Vm[:1400,:] for i in range(repeat)], axis=1))
    count += 1
    if count % 50 == 0:
...
if Normalization:  
    ...
    X_array      = np.concatenate((scale_factor*(X_array_old[:,:repeat*68] - 60), 10*(X_array_old[:,repeat*68:] - 1)), axis = 1) 
    Y_array      = np.concatenate((scale_factor*(Y_array_old[:,:repeat*68] - 60), 10*(Y_array_old[:,repeat*68:] - 1)), axis = 1) 
    U_array      = np.concatenate((scale_factor*(U_array_old[:,:repeat*68] - 60), 10*(U_array_old[:,repeat*68:] - 1)), axis = 1) 
    V_array      = np.concatenate((scale_factor*(V_array_old[:,:repeat*68] - 60), 10*(V_array_old[:,repeat*68:] - 1)), axis = 1) 
    Yp_array     = np.concatenate((scale_factor*(Yp_array_old[:,:repeat*68] - 60), 10*(Yp_array_old[:,repeat*68:] - 1)), axis = 1)
    Yf_array     = np.concatenate((scale_factor*(Yf_array_old[:,:repeat*68] - 60), 10*(Yf_array_old[:,repeat*68:] - 1)), axis = 1)    
