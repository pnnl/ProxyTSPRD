...
def get_data(t: tf.string):
    ...
    raw_data = np.concatenate([dataset.F for i in range(repeat)] +  
                              [dataset.Vm for i in range(repeat)], axis=1)
...
def convert_to_tensor(i):
    ...
    d.set_shape(tf.TensorShape([1400, repeat*136]))
    return d
...
_VERSION = sys.argv[2]
_LABEL = '_TFDataOptMPMGPU'
_SUFFIX = 'e' + str(_N_EPOCHS) + _LABEL + "_g" + sys.argv[3] + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
keepN = 1400
...
with options({'constant_folding': True}):
    ...
    if normalization:
        flat_Yp_data = flat_Yp_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                             tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
        flat_Yf_data = flat_Yf_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                             tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
        flat_X_data = flat_X_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                           tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
        flat_Y_data = flat_Y_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                           tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
        flat_U_data = flat_U_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                           tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
        flat_V_data = flat_V_data.map(lambda x: tf.concat([tf.math.multiply(tf.math.subtract(x[:repeat*68], 60), scale_factor), \
                                                           tf.math.multiply(tf.math.subtract(x[repeat*68:], 1), 10)], axis=0),
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)    
    ...
    hyper_param_dict['original_dim']       = repeat*136   # input data dimension
