...
class NeuralNetworkModel(): 
    def __init__(self, hp, model, optimizer, device):
        # model and optimizer
        self.encoder = model
    ...
    def predict(self, input_dataset):
        for X, Y in input_dataset:
            ...
            return Psi_X.cpu().data.numpy(), PSI_X.cpu().data.numpy(), Psi_Y.cpu().data.numpy(), PSI_Y.cpu().data.numpy(), K_loss.cpu().data.numpy()
