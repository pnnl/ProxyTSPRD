...
def main():
    ...
    def w_loop_0_0():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, 'kPredicateDrop', 32, 0, [4096, 1, ])
        np.save('RVRM_w_loop_0_0.npy', matrix)
    if opts.ctx == 'w_loop_0_0':
        w_loop_0_0()

    def kBackReadCtx():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, 'kPredicateForceZero', 32, 0, [4096, 1, ])
