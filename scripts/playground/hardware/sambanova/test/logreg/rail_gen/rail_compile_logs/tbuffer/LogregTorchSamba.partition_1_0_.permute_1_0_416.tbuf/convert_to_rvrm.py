...
def main():
    ...
    def W():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, 'kPredicateForceZero', 32, 3, [256, 3, 1, 160, ])
        np.save('RVRM_W.npy', matrix)
    if opts.ctx == 'W':
        W()

    def R():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, 'kPredicateDrop', 32, 0, [256, 3, 1, 160, ])
