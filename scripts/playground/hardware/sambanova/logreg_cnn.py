...
class PTConvCNN(torch.nn.Module):
    def __init__(self, bw_size, fw_size, n_features):
        super(PTConvCNN, self).__init__()
    ...
    def forward(self, inputs, targets):
        ...
        # print(out[:, 0, :].shape, targets.reshape(1, -1).shape)
        # sys.exit("checks")
        loss = self.criterion(
            out.reshape(-1, self.fw_size*self.n_features), 
            targets.reshape(-1, self.fw_size*self.n_features))#.item()
        # print(l)
        # return loss, out
