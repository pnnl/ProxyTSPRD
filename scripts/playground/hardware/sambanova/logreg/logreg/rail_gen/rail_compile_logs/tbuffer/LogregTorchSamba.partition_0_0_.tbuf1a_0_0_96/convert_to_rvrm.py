...
def main():
    ...
    def kDefaultWrite():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [10, 512, ])
        np.save('RVRM_kDefaultWrite.npy', matrix)
    if opts.ctx == 'kDefaultWrite':
        kDefaultWrite()

    def LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXVec_0():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [10, 512, ])
        np.save('RVRM_LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXVec_0.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXVec_0':
        LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXVec_0()

    def LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXScalar():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, -1, [10, 512, ])
        np.save('RVRM_LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXScalar.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXScalar':
        LogregTorchSamba.partition_0_0_.logreg__criterion__crossentropyloss@kInputXScalar()
