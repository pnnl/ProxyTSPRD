...
def ROB_kBackReadCtx_0_0_kBackReadCtx_0_0__dup0_W():
    ...
    print("wt byte:{} word:{} vec:{} rotate:{} pred_bits:{} zero:{} pred:{} invalid:{} min_max:{}".format(qaddr(x13),qaddr(x13//2),qaddr(x13//64),qaddr(x3),bin(x4)[2::],x4 % 2 >> 0 ,x4 % 4 >> 1 ,x4 % 8 >> 2 ,True,))
    return ndaddrs
...
def ROB_kBackReadCtx_0_0_kBackReadCtx_0_0__dup0_R():
    ...
    for x9 in range(x4,x3,x2):
        ...
        x10 = True
        x11 = x10 | x7
        print("rd byte:{} word:{} vec:{} rotate:{} pred_bits:{} zero:{} pred:{} invalid:{} min_max:{}".format(qaddr(x9),qaddr(x9//2),qaddr(x9//64),qaddr(x4),bin(x5)[2::],x5 % 2 >> 0 ,x5 % 4 >> 1 ,x5 % 8 >> 2 ,True,))
    return ndaddrs
