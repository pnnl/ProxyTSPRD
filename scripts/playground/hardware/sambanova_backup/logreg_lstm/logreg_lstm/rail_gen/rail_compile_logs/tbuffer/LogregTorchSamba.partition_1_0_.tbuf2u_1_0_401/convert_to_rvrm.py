...
def main():
    ...
    def kDefaultWrite():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [60, 64, ])
        np.save('RVRM_kDefaultWrite.npy', matrix)
    if opts.ctx == 'kDefaultWrite':
        kDefaultWrite()

    def LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kLutRd():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [60, 64, ])
        np.save('RVRM_LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kLutRd.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kLutRd':
        LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kLutRd()
