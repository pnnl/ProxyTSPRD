...
def kDefaultWrite():
    ...
    x5 = 64
    ndaddrs = []
    for x6 in range(x4,x3,x5):
        x7 = True
        print("trim_pred: {} ".format(x1), end='')
        print("pmufid: {} ".format(x4), end='')
        print("wt byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x6),qaddr(x6//2),qaddr(x6//64),qaddr(x4),qaddr(x4),x1,x1,True,))
    return ndaddrs
...
def LogregTorchSamba_partition_1_0__ptconvlstm__indexselect_recompute__kLutRd():
    ...
    for x7 in range(x4,x3,x2):
        ...
        x13 = x7 + x12
        print("pmufid: {} ".format(x4), end='')
        print("rd byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x13),qaddr(x13//2),qaddr(x13//64),qaddr(x4),qaddr(x4),x10,x1,True,))
    return ndaddrs
