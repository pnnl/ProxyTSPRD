...
def main():
    ...
    def kFrontDynamicWriteCtx():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [60, 160, ])
        np.save('RVRM_kFrontDynamicWriteCtx.npy', matrix)
    if opts.ctx == 'kFrontDynamicWriteCtx':
        kFrontDynamicWriteCtx()

    def LogregTorchSamba.partition_1_0_.ptconvcnn__lambda_layer__indexselect_recompute_@kLutRd():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 32, 1, [60, 160, ])
        np.save('RVRM_LogregTorchSamba.partition_1_0_.ptconvcnn__lambda_layer__indexselect_recompute_@kLutRd.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_1_0_.ptconvcnn__lambda_layer__indexselect_recompute_@kLutRd':
        LogregTorchSamba.partition_1_0_.ptconvcnn__lambda_layer__indexselect_recompute_@kLutRd()
