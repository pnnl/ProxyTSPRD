...
def kBackReadCtx_0_0__dup0_R():
    ...
    for x6 in range(x3,x2,x5):
        for x7 in range(x3,x5,x4):
            ...
            x12 = x11 * x1
            print("pmuid: [" + str(x3) +"," + str(x3) +"," + "] ", end='')
            print("rd byte:{} word:{} vec:{} rotate:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x12),qaddr(x12//2),qaddr(x12//64),qaddr(x3),x0,x0,True,))
    return ndaddrs
