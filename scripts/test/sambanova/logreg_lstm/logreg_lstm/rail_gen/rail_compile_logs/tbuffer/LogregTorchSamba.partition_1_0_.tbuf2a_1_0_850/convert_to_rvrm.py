...
def main():
    ...
    def kDefaultWrite():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 16, 0, [1, ])
        np.save('RVRM_kDefaultWrite.npy', matrix)
    if opts.ctx == 'kDefaultWrite':
        kDefaultWrite()

    def LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kIndexRd():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 16, -1, [1, ])
        np.save('RVRM_LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kIndexRd.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kIndexRd':
        LogregTorchSamba.partition_1_0_.ptconvlstm__indexselect_recompute_@kIndexRd()
