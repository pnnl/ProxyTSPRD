...
def LogregTorchSamba_partition_0_0__logreg__criterion__crossentropyloss_kOutput():
    ...
    for x5 in range(x3,x1,x1):
        ...
        print("wt byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x3),qaddr(x3//2),qaddr(x3//64),qaddr(x3),qaddr(x3),x2,x2,True,))
    return ndaddrs
...
def kBackReadCtx():
    ...
    for x4 in range(x3,x1,x0):
        x5 = True
        print("pmuid: [" + str(x3) +"," + "] ", end='')
        print("rd byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x3),qaddr(x3//2),qaddr(x3//64),qaddr(x3),qaddr(x3),x2,x2,True,))
    return ndaddrs
