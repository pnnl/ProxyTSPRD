...
def main():
    ...
    def kFrontDynamicWriteCtx():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 16, 0, [3, ])
        np.save('RVRM_kFrontDynamicWriteCtx.npy', matrix)
    if opts.ctx == 'kFrontDynamicWriteCtx':
        kFrontDynamicWriteCtx()

    def LogregTorchSamba.partition_1_0_.ptconvlstm__lambda_layer__indexselect_recompute_@kIndexRd():
        matrix = crvrm.convert_to_rvrm(addrs, opts.data_file, '', 16, -1, [3, ])
        np.save('RVRM_LogregTorchSamba.partition_1_0_.ptconvlstm__lambda_layer__indexselect_recompute_@kIndexRd.npy', matrix)
    if opts.ctx == 'LogregTorchSamba.partition_1_0_.ptconvlstm__lambda_layer__indexselect_recompute_@kIndexRd':
        LogregTorchSamba.partition_1_0_.ptconvlstm__lambda_layer__indexselect_recompute_@kIndexRd()
