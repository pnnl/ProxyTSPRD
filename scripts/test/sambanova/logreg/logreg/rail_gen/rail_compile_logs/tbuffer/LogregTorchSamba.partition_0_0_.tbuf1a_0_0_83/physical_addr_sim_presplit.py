...
def D_0_0_LogregTorchSamba_partition_0_0__logreg__criterion__crossentropyloss_kOutput():
    ...
    x4 = 0
    ndaddrs = []
    for x6 in range(x2,x1,x1):
        x7 = True
        print("wt byte:{} word:{} vec:{} rotate:{} metadata:{} pred_bits:{} zero:{} pred:{} invalid:{} min_max:{}".format(qaddr(x2),qaddr(x2//2),qaddr(x2//64),qaddr(x2),qaddr(x2),bin(x3)[2::],x3 % 2 >> 0 ,x3 % 4 >> 1 ,x3 % 8 >> 2 ,True,))
    return ndaddrs
...
def D_0_0_kBackReadCtx():
    ...
    for x5 in range(x2,x1,x0):
        x6 = True
        print("rd byte:{} word:{} vec:{} rotate:{} metadata:{} pred_bits:{} zero:{} pred:{} invalid:{} min_max:{}".format(qaddr(x2),qaddr(x2//2),qaddr(x2//64),qaddr(x2),qaddr(x2),bin(x4)[2::],x4 % 2 >> 0 ,x4 % 4 >> 1 ,x4 % 8 >> 2 ,True,))
    return ndaddrs
