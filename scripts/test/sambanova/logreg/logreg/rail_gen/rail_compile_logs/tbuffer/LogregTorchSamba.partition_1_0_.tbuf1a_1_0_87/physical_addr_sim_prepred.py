...
def D_0_0_kDefaultWrite():
    ...
    x6 = 0
    ndaddrs = []
    for x7 in range(x6,x4,x2):
        x8 = True
        print("wt byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x7),qaddr(x7//2),qaddr(x7//64),qaddr(x6),qaddr(x6),x5,x5,True,))
    return ndaddrs
...
def D_0_0_LogregTorchSamba_partition_1_0__logreg__criterion__crossentropyloss_bwd_loss_kInputX0():
    ...
    x6 = 0
    ndaddrs = []
    for x7 in range(x6,x1,x0):
        for x8 in range(x6,x3,x2):
            x9 = True
            x12 = x8 < x4
            print("rd byte:{} word:{} vec:{} rotate:{} metadata:{} en_zero:{} en_pred:{} min_max:{}".format(qaddr(x8),qaddr(x8//2),qaddr(x8//64),qaddr(x6),qaddr(x6),x12,x5,True,))
            x14 = True
            x17 = x15 | x16
    return ndaddrs
