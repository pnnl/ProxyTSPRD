...
class PTCNN2D(torch.nn.Module):
  def __init__(self, model_parameters):
      ...
      # self.criterion = criterion
      self.conv_layer    = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(self.bw_size, self.bw_size))
...
class AIT_PTCNN(nn.Module):
  def __init__(self, model_parameters):
      ...
      self.conv_layer    = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=self.bw_size, stride=1)
  ...
  def forward(self, inputs):
      ...
      reshaped_input = self.reshape_layer(inputs, (1024, self.bw_size, self.n_features, 1))
      print("Reshaped Inputs---------------", reshaped_input.shape())
      c = self.conv_layer(reshaped_input)
      print("Conv---------------", c.shape())
...
x1d = torch.randn([batch_size, model_parameters["bw_size"], model_parameters["n_features"]])#.cuda()
print(x1d.shape)
...
x2d = torch.randn([batch_size, 1, model_parameters["bw_size"], model_parameters["n_features"]])#.cuda()
print(x2d.shape)
...
X = Tensor(
      shape=[batch_size, model_parameters["bw_size"], model_parameters["n_features"]],
      name="X",
      is_input=True,
)
...
#     y = torch.empty([batch_size, model_parameters["fw_size"], model_parameters["n_features"]]).cuda()
#     print("Empty y:", y)
