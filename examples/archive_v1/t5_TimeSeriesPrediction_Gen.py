...
class DatasetGeneratorV2(tf.data.Dataset):
    def _generator(X, Y, scale_factor, norm):
        if X.shape[0] == Y.shape[0]: 
            ...
            for i in range(d_shape):
                if norm:
                    x_norm = np.concatenate((scale_factor*(X[i,:repeat*68] - 60), 10*(X[i,repeat*68:] - 1))) 
                    y_norm = np.concatenate((scale_factor*(Y[i,:repeat*68] - 60), 10*(Y[i,repeat*68:] - 1))) 
                    yield x_norm, y_norm
                else:
                    yield X[i], Y[i]
    ...
    def __new__(self, X, Y, scale_factor, norm=True):
        return tf.data.Dataset.from_generator(
            ...
            output_shapes = ((repeat*136,), (repeat*136,)),
...
for s_dir in dir_list:
    ...
    scenario_data.append(np.concatenate([dataset.F[:keepN,:] for i in range(repeat)] + 
                                        [dataset.Vm[:keepN,:] for i in range(repeat)], axis=1))
    count += 1
    if count % 50 == 0:
